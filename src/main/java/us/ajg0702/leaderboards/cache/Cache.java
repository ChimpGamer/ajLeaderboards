...
public class Cache {
    ...
    public StatEntry getStat(int position, String board, TimedType type) {
        ...
        try (Connection conn = method.getConnection();
            ...
            return processData(rs, sortBy, position, board, type);
    ...
    public boolean createBoard(String name) {
        try (Connection conn = method.getConnection();
            ...
            for (TimedType type : TimedType.values()) {
                ...
                try (PreparedStatement ps1 = conn.prepareStatement(method.formatStatement(String.format(
    ...
    public void updateStat(String board, OfflinePlayer player) {
        ...
        Runnable updateTask = () -> {
            ...
            if (debug)
                Debug.info("Updating " + player.getName() + " on board " + board + " with values v: " + output + " suffix: " + finalSuffix + " prefix: " + finalPrefix);
            try (Connection conn = method.getConnection()) {
    ...
    private StatEntry processData(ResultSet rs, String sortBy, int position, String board, TimedType type) throws SQLException {
        String uuidRaw = null;
        double value = -1;
        String name = "-Unknown-";
        String displayName = name;
        ...
        try {
            ...
            suffix = rs.getString(5);
            displayName = rs.getString(6);
            value = rs.getDouble(dataSortByIndexes.computeIfAbsent(sortBy,
                    k -> {
                        try {
